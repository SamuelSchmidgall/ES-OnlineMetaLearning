...
def compute_returns(seed, environment, network, num_eps_samples, K=1, noise_eps=0.01):
    """
    Compute returns for ES-MAML algorithm
    :param seed:
    :param environment:
    :param network:
    ...
    for _sample in range(eps_samples.shape[0]):
        return_avg = 0.0
        local_env.env.set_task(_sample)
        ...
        return_avg /= K
        new_eps_delta = 0.05*(1 / noise_eps)\
                  * return_avg * eps_samples[_sample]
        eps_samples[_sample] += new_eps_delta
        network = deepcopy(network_cpy)
        network.update_params(eps_samples[_sample])
...
class EvolutionaryOptimizer:
    ...
    def parallel_returns(self, x):
        ...
        return compute_returns(seed=seed, environment=self.environments[worker_id],
            ...
            noise_eps=self.network.es_optim.noise_std, K=20)
    ...
    def update(self, iteration):
        ...
        change_mu = (1 / (self.epsilon_samples
            * self.network.es_optim.noise_std)) * np.dot(eps.T, returns)
...
if __name__ == "__main__":
    ...
    es_optim = EvolutionaryOptimizer(
        ...
        learning_rate=0.01, epsilon_samples=300, num_workers=2)
